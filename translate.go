...
type sentence struct {
	...
	Translit string `json:"src_translit"`
}
...
func (a *Translator) Transliterate(origin, src, dest string) (*Translated, error) {
	// check src & dest
	src = strings.ToLower(src)
	dest = strings.ToLower(dest)
	//if _, ok := languages[src]; !ok {
	...
	text, err := a.transliterate(a.client, origin, src, dest)
	if err != nil {
		return nil, err
	}
	result := &Translated{
...
func (a *Translator) transliterate(client *http.Client, origin, src, dest string) (string, error) {
	tk, err := a.ta.do(origin)
	if err != nil {
		return "", err
	}
